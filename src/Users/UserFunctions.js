...
async function signUpUser(userDetails){
    ...
    return firebaseAdmin.auth().createUser({ // use firebaseAdmin create a user
        ...
        emailVerified: true, // Required for fuller user functionality, but a hassle to set up in a short tutorial. Set to false
        // if you do end up configuring email verifications, as the email system will set it to true.
        ...
        let defaultUserClaims = firebaseAdmin.auth().setCustomUserClaims(userRecord.uid, {regularUser: true}).then(() => {
            ...
            // You can do things like detect values in the email address (eg. if the new user email is the project admin email)
            //  and set the claim object to include other values.
            // Claims allow you to handle authorization without ever giving the client any data that they could hack or manipulate.
            // Of course, you can still pass the claims along to the client if you want to (eg. for front-end authorization to hide content), 
            // just know that front-end authorization isn't bulletproof.
